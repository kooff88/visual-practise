...
import { D3ExponentialSpiralLineObj } from "./interface";
import styles from './index.less';


const D3ExponentialSpiralLine: React.FC<{}> = (props) => {
    let { a } = props;

    const [exponentialSpiralLine, setExponentialSpiralLine] = useState<D3ExponentialSpiralLineObj>({
        width: null,
        height: null,
        outerRadius: null,
        keys: null,
        fy: (x, a = 2 ) => Math.pow(a, x)  // y=a^x (a为常数且以a>0，a≠1)
    });
    console.log('a',a)
    ...
    const drawMap = () => {
        ...
        let fy = exponentialSpiralLine.fy;
      ...
      d3.range(0, 1500, 1).forEach(function(item) {
      ...
      let x = d3
        ...
        .align(0)
        .domain(keys)
      let y = quarticScaleRadial()
        .range([0, outerRadius])
        .domain([0, 100000000]) // 定义y轴
    ...
    const updateMap = () => { 
        ...
        let { outerRadius, fy } = exponentialSpiralLine;
        a = a > 0 && a !== 1 ? a : 2;

        let points = []
        let formatXY = d3.format('.2f');
        d3.range(5, 46.5, 0.02).forEach((item) => { 
            const angle = formatXY(item);
            const radius = formatXY(fy(angle, a))
            points.push([angle, radius])
        })
    ...
    console.log("exponentialSpiralLine",exponentialSpiralLine)
...
export default D3ExponentialSpiralLine;
