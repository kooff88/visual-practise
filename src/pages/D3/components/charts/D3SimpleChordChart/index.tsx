...
const D3SimpleChordChart: React.FC<{}> = (props) => {
  let { data: { names, matrix } } = props;
    ...
    const drawMap = () => {
      const container = document.getElementById("containerSCC")
      const containerWidth = container.parentElement.offsetWidth
      const margin = { top: 100, right: 60, bottom: 100, left: 60 }

      const width = containerWidth - margin.left - margin.right
      const height = 700 - margin.top - margin.bottom
      const outerRadius = Math.min(width, height) * 0.5 - 40; //设置半径（背线）
      const innerRadius = outerRadius - 30; //设置半径（基线）
